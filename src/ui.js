...
let sortKey='champ',sortDir=-1,lastStat=null,lastN=0;
function render(stat,N){lastStat=stat;lastN=N;document.getElementById('simcount').textContent=N.toLocaleString('es')+' sims';const arr=ALL_TEAMS.map(t=>({t,c:stat[t].champ/N})).sort((a,b)=>b.c-a.c);const max=arr[0].c||1;const cb=document.getElementById('champbars');cb.innerHTML='';arr.slice(0,12).forEach((o,i)=>{const row=document.createElement('div');row.className='cbar';row.innerHTML=`<div class="nm"><span class="rk">${i+1}</span>${o.t}</div><div class="track"><div class="fill" style="width:${(o.c/max*100).toFixed(1)}%"></div></div><div class="pct">${(o.c*100).toFixed(1)}%</div>`;cb.appendChild(row);});renderTable();}
function renderTable(){if(!lastStat)return;const N=lastN;const rows=ALL_TEAMS.map(t=>({name:t,grp:teamGroup[t],rating:ratings[t],r16:lastStat[t].r16/N,qf:lastStat[t].qf/N,sf:lastStat[t].sf/N,fin:lastStat[t].fin/N,champ:lastStat[t].champ/N}));rows.sort((a,b)=>{let va=a[sortKey],vb=b[sortKey];if(sortKey==='name')return sortDir*va.localeCompare(vb);if(sortKey==='grp')return sortDir*(va<vb?-1:va>vb?1:0);return sortDir*(va-vb);});const tb=document.getElementById('rbody');tb.innerHTML='';rows.forEach(r=>{const tr=document.createElement('tr');const cell=v=>{const pct=v*100;return `<td class="mono heat ${pct>=25?'hot':''}">${pct<0.05?'·':pct.toFixed(1)}</td>`;};const host=HOSTS.has(r.name)?' <span class="hostdot">HOST</span>':'';tr.innerHTML=`<td class="team"><span class="gtag" style="background:${GROUP_COLORS[r.grp]}">${r.grp}</span>${r.name}${host}</td><td class="mono" style="color:var(--mut)">${r.grp}</td><td class="mono" style="color:var(--mut)">${r.rating}</td>${cell(r.r16)}${cell(r.qf)}${cell(r.sf)}${cell(r.fin)}<td class="mono" style="color:var(--accent);font-weight:600">${(r.champ*100)<0.05?'·':(r.champ*100).toFixed(1)}</td>`;tb.appendChild(tr);});document.querySelectorAll('#rtable th').forEach(th=>th.classList.toggle('sorted',th.dataset.k===sortKey));}
...
document.querySelectorAll('.tab').forEach(t=>t.addEventListener('click',()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));t.classList.add('active');['prob','quin','valid','ratings'].forEach(id=>document.getElementById('tab-'+id).classList.toggle('hidden',id!==t.dataset.tab));if(t.dataset.tab==='valid')runValidation();window.scrollTo({top:0,behavior:'smooth'});}));
